...
public class BookingService {
    ...
    public Booking bookTour(String email, Long tourId) {
        ...
        if (tour.getAvailablePlaces() <= 0) {
            throw new BusinessException("No available places for this tour");
        }
        if (!tour.getStartDate().isAfter(LocalDate.now())) {
            throw new BusinessException("This tour has already started");
        }
        if (bookingRepository.existsByUserAndTourAndStatusNot(user, tour, BookingStatus.CANCELED)) {
            throw new BusinessException("You have already booked this tour");
        }
        ...
        log.info("Tour booked: bookingId={} user={} tour={}", saved.getId(), email, tourId);
        return saved;
    }
    ...
    public void payForBooking(String email, Long bookingId) {
        ...
        if (booking.getStatus() != BookingStatus.REGISTERED) {
            throw new BusinessException("Only registered bookings can be paid");
        }
    ...
    public void cancelUserBooking(String email, Long bookingId) {
        ...
        if (!booking.getTour().getStartDate().isAfter(LocalDate.now())) {
            throw new BusinessException("Tour has already started, cancellation is not allowed");
        }
